...
public class Serwer
{
   ...
   public void startSerwer()
   {
	   try {
		...
		while(true)
		{
			...
			Thread thread = new Thread(new ServerKlient(socket,printWriter)); //stworzenie nowego w¹tku dla niego
   ...
   class ServerKlient implements Runnable
   {
	...
	public ServerKlient(Socket socket,PrintWriter printWriter) 
	{
		try {
		this.socked=socket;
